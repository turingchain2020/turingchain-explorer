...
export default {
  ...
  methods: {
    ...
    getblockDetail: function() {
      this.$chainRpc.getBlockHash(Number(this.data.Height)).then(data => {
        if (data.error == null) {
    ...
    getBlockInfo: function(hash) {
      //获取概况
      this.$chainRpc.getBlockOverview(hash).then(data => {
        if (data.error == null) {
    ...
    getBlockTable: function() {
      ...
      this.$chainRpc.getTxByHashes(hasharrlist).then(data => {
        if (data.error == null) {
