...
export default {
  ...
  methods: {
    ...
    getSuccessToken() {
      let symbolArr = []
      let tokenArr = []
      this.$chainRpc.queryAllTokens().then(data => {
    ...
    // 获取地址信息
    getAdrInfo() {
      this.$chainRpc.getAddrOverview(this.addressInfo.adr).then(data => {
        if (this.checkRequest(data)) {
    ...
    getTokenAssets() {
      let tokens = []
      let infos = []
      this.$chainRpc.getAddrTokenAssets(this.addressInfo.adr, 'token').then(data => {
        if (data.error === null) {
    ...
    getTxList(page) {
      ...
      if (this.chooseSymbol.symbol === '全部') {
        ...
        //根据token获取交易
        this.$chainRpc.getTxByToken(this.Params).then(res => {
    ...
    getLastHash() {
      ...
      if (Param.symbol === '') {
        this.$chainRpc.getTxByAddr(Param).then(data => {
          if (this.checkRequest(data)) {
            this.lastTrade = data.result.txInfos[0].hash
          }
        })
      } else {
        this.$chainRpc.getTxByToken(Param).then(data => {
          if (this.checkRequest(data)) {
            this.lastTrade = data.result.txInfos[0].hash
          }
        })
