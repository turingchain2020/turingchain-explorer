...
export default {
  ...
  methods: {
    ...
    getLastBlock() {
      ...
      this.isLoading = true;
      this.$chainRpc.getLastHeader().then((data) => {
        if (data.error === null) {
    ...
    getBlocksHash() {
      ...
      this.$chainRpc.getHeaders(this.startHeight, this.endHeight).then((data) => {
        if (data.error === null) {
    ...
    getTradeHashes(hashArr) {
      ...
      let promises = hashArr.map((current) => {
        return _this.$chainRpc.getBlockOverview(current).then((data) => {
          if (data.error === null) {
      ...
      let tradeDetail = Promise.all(promises).then((res) => {
        ...
        let tradeInfo = []
        //根据hashes数组获取交易详情
        let e = this.$chainRpc.getTxByHashes(tradeHashArr).then((data) => {
          if (data.error === null) {
