    ...
    submit() {
      ...
      if (reg.test(this.Value.trim()) || this.Value == 0) {
        if (Number(this.Value.trim()) <= Number(this.height)) {
          //判断两个高度差---int类型比较
          this.$chainRpc.getBlockHash(Number(this.Value)).then(data => {
            if (data.error == null) {
              this.$router.push({
                path: "/blockDetail",
        ...
        if (e === 66 || e === 64) {
          ...
          this.$chainRpc.queryTransaction(this.Value).then(data => {
            if (data.error == null) {
              this.$router.push({
                path: "/tradeHash",
              ...
              //区块哈希
              this.$chainRpc.getBlockOverview(this.Value).then(data => {
                if (data.error == null) {
                  this.$router.push({
                    path: "/blockDetail",
          ...
          //地址
          this.$chainRpc.getAddrOverview(this.Value).then(data => {
            if (data.error == null) {
              this.$router.push({
                path: "/addressDetail",
